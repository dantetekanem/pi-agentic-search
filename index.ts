...
export function formatSearchResults(query: string, ranked: RankedFileResult[], totalMatches: number): string {
  if (ranked.length === 0) return `No matches found for ${JSON.stringify(query)}.`;
  ...
  const lines = [
    ...
    `TARGET FILE: ${ranked[0]?.path}. Read only this file before editing when it contains the requested construct matches.`,
    "",
  ];
  ...
  ranked.forEach((file, index) => {
    ...
    lines.push(`${index + 1}. ${file.path} (score ${file.score}${candidateConfidence}, ${file.matchCount} match${file.matchCount === 1 ? "" : "es"})${reasonText}`);

    if (file.matches.length === 0) {
      lines.push("   [path] filename/path match");
    }

    for (const match of file.matches) {
      const topMatch = formatTopMatch(match);
      lines.push(`   L${topMatch.lineNumber} [${topMatch.marker}] ${topMatch.text}`);
    }

    if (file.matches.length > 0 && file.matchCount > file.matches.length) {
      lines.push(`   … ${file.matchCount - file.matches.length} more match${file.matchCount - file.matches.length === 1 ? "" : "es"} in this file`);
    }

    lines.push("");
  });

  lines.push("Next step: read only the TARGET FILE, then propose the edit from that file. Do not inspect alternate candidates, sibling models, tests, migrations, git status, or run more searches unless the target file lacks the requested construct matches.");
  return lines.join("\n").trimEnd();
}
...
async function existingSearchRoot(cwd: string, candidate: string): Promise<string | undefined> {
  try {
    const resolved = ensureInsideCwd(cwd, stripAtPrefix(candidate));
    await stat(resolved);
    return normalizeRepoRelativePath(relative(cwd, resolved)) || ".";
  } catch {
    return undefined;
  }
}
...
async function resolveSearchScope(params: {
  ...
  if (existing) {
    return {
      searchRoots: [existing],
      pathMatches: await listPathMatches({ cwd: params.cwd, query: pathQuery, searchRoot: existing, signal: params.signal }),
    };
  }

  const pathMatches = await listPathMatches({ cwd: params.cwd, query: pathQuery, searchRoot: ".", signal: params.signal });
  const searchRoots = pathMatches.slice(0, Math.max(params.maxFiles, 1)).map((match) => match.path);
...
export default function agenticSearchExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description: `Preferred ranked search for locating files, classes, scopes, methods, and call sites across a repository. Uses ripgrep, then ranks and groups results for coding-agent workflows. Output is truncated to ${DEFAULT_MAX_LINES} lines or ${formatSize(DEFAULT_MAX_BYTES)}.`,
    promptSnippet: "Preferred ranked search for locating files, classes, scopes, methods, and call sites across a repository. Make one focused search from the user's named construct and optional path hint; if the target file contains the requested matches, stop discovery and read only that file before editing.",
    promptGuidelines: [
      ...
      "For Rails scope requests like predicates for scopes on event_occurrence.rb, call agentic_search once with query scope\\s+: and path event_occurrence.rb, then read only the target file before editing.",
      "If agentic_search returns a target file containing the requested construct matches, stop discovery: do not inspect alternate candidates, sibling models, tests, migrations, git status, or run more searches before proposing the edit.",
      "Use alternate candidates only if the target file is missing, ambiguous, or lacks the requested construct matches.",
    ],
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const buildArgs = (literal: boolean) => {
        ...
        args.push(params.query, ...scope.searchRoots);
        return args;
      };
      ...
      const ranked = withConfidence(rankFileGroups(matches, params.path ?? params.query, maxMatchesPerFile, pathMatches).slice(0, maxFiles));
      ...
      const formatted = `${formatSearchResults(params.query, ranked, totalMatches)}${fallbackNotice}`;
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      if (top?.path) {
        ...
        text += theme.fg("muted", " → read only target first");
      }
      ...
      if (expanded && files.length > 0) {
        for (const file of files.slice(0, 10)) {
          ...
          text += `\n${theme.fg("accent", file.path)} ${theme.fg("dim", `(score ${file.score}${confidence}, ${file.matchCount} matches)${topMatch}`)}`;
        }
  ...
  pi.registerCommand("agentic-search-info", {
    ...
    handler: async (_args, ctx) => {
      ctx.ui.notify(
        "pi-agentic-search loaded: agentic_search only. For named-file construct matches, read only the target file before editing.",
        "info",
      );
    },
  });
